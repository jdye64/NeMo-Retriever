# About getting started

This section walks you from **access and prerequisites** through **first deployment** and **hands-on notebooks**.

Typical order:

1. [Get your API key](api-keys.md) (NGC / API access as required by your workflow).
2. Confirm the [Pre-Requisites & Support Matrix](prerequisites-support-matrix.md) for your OS, GPU, and software stack. Local GPU inference requires Linux; remote NIM workflows can use the base package on Windows x64 and macOS Apple Silicon (arm64) as well. macOS Intel (x86_64) is not supported.
3. Choose a path in [Deployment options](deployment-options.md). You can use the local library, hosted NIMs, or a standalone Docker service.
4. Explore [Jupyter Notebooks](https://github.com/NVIDIA/NeMo-Retriever/blob/26.08.1/examples/README.md) for end-to-end examples.

The NeMo Retriever Library is not supported under NVIDIA AI Enterprise (NVAIE). For more information, refer to [NVIDIA AI Enterprise (NVAIE) support](overview.md#nvidia-ai-enterprise-nvaie-support).

If you are new to the product, read [NeMo Retriever Library Overview](overview.md) and [Concepts](concepts.md) under **Introduction** first.
