...
No. The NeMo Retriever Library, including its container image, is not supported under NVIDIA AI Enterprise (NVAIE).
...
For examples of `*_ENDPOINT` variables, refer to [Environment variables](environment-config.md).
