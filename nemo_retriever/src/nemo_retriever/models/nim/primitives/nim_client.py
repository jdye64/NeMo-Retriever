...
from nemo_retriever.common.tracing import record_model
...
class NimClient:
    ...
    def infer(self, data: dict, model_name: str, **kwargs) -> Any:
        ...
        """
        # Record which model answered so the page trace can report its version.
        # The call is not timed separately; the enclosing operator span covers it.
        record_model(
            self.model_interface.name(),
            name=model_name,
            backend=f"nim-{self.protocol}",
            endpoint=self._grpc_endpoint if self.protocol == "grpc" else self._http_endpoint,
        )
