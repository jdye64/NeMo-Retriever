            ...
            try:
                ...
                try:
                    ...
                    if want_any_raster:
                        ...
                        # Raster area sets the floor on every downstream image
                        # stage, so it explains an unusually expensive page.
                        orig_h, orig_w = render_info["orig_shape_hw"]
                        record_page_work(
                            source_id,
                            rendered_megapixels=round(float(orig_h) * float(orig_w) / 1e6, 4),
                        )
